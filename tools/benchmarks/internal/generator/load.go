...
// ActionRequest matches the gateway's models.ActionRequest schema for
// POST /v1/runtime/check so benchmark traffic exercises the real path.
...
func (g *LoadGenerator) SelectAction(rng *rand.Rand) ActionRequest {
	...
	newReq := func(actionType, resource, env string) ActionRequest {
		return ActionRequest{
			ActionType:  actionType,
			Resource:    resource,
			Environment: env,
			Nonce:       fmt.Sprintf("bench-%d-%d", time.Now().UnixNano(), rng.Intn(1<<30)),
	...
	switch {
	case r < 40: // 40%
		return newReq("shell", "shell:echo test", "local")
	case r < 70: // 30%
		return newReq("git.pull", "git:ovara-main", "dev")
	case r < 90: // 20%
		return newReq("git.push", "git:ovara-main:feature/benchmark", "staging")
	default: // 10%
		return newReq("exec", "exec:ls -la", "local")
	}
}

func (g *LoadGenerator) sendRequest(action ActionRequest) bool {
