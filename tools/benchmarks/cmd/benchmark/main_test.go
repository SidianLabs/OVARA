...
func TestActionSelection(t *testing.T) {
	...
	if counts["shell"] < 3500 || counts["shell"] > 4500 {
		t.Errorf("Expected shell ~4000, got %d", counts["shell"])
	}
	...
	if counts["exec"] < 500 || counts["exec"] > 1500 {
		t.Errorf("Expected exec ~1000, got %d", counts["exec"])
	}
}
